...
func newPodResourceWatcher(informerFactory informers.SharedInformerFactory, logger log.Logger) *watcher {
	return &watcher{
...
func (w *watcher) handleEvent(ctx context.Context, obj interface{}, updater ResourceUpdater) {
	...
	case *corev1.ConfigMap:
		...
		w.mu.RUnlock()
		if len(pods) != 0 {
			w.log.Infof("got ConfigMap update %s/%s, notifying %d pods", v.Namespace, v.Name, len(pods))
		}
		for _, pod := range pods {
			if err := updater.UpdateConfigMap(ctx, pod, v); err != nil {
				w.log.Warnf("failed to update ConfigMap %s/%s in Pod %s: %s", v.Namespace, v.Name, pod.Name, err)
			}
		}
	...
	case *corev1.Secret:
		...
		w.mu.RUnlock()
		if len(pods) != 0 {
			w.log.Infof("got Secret update %s/%s, notifying %d pods", v.Namespace, v.Name, len(pods))
		}
		for _, pod := range pods {
			if err := updater.UpdateSecret(ctx, pod, v); err != nil {
				w.log.Warnf("failed to update Secret %s/%s in Pod %s: %s", v.Namespace, v.Name, pod.Name, err)
			}
		}
	default:
		w.log.Warnf("ignoring update to resource of unsupported type %T", v)
	}
}
