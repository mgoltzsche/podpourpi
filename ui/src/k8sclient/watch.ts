...
export function watch<T>(url: string, handler: (evt: WatchEvent<T>)=>void): void {
  streamFromURL<WatchEvent<T>>(url).then(stream => {
    ...
    setTimeout(() => {watch(url, handler)}, 1000) // retry after 1s
  })
}
